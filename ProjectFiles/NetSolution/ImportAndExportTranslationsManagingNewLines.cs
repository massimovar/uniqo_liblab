...
public class ImportAndExportTranslationsManagingNewLines : BaseNetLogic
{
    string NEW_LINE_PLACEHOLDER = "\\n";
            ...
            using (var csvWriter = new CSVFileWriter(csvPath) { FieldDelimiter = characterSeparator.Value, WrapFields = wrapFields })
            {
                for (var r = 0; r < rowCount; ++r)
                {
                    ...
                    for (var c = 0; c < columnCount; ++c) {
                        ...
                        row[c] = ReplaceNewLineWithSymbol(dictionary[r, c]);
                    }
    ...
    private string ReplaceSymbolWithNewLine(string i) => i.Replace(NEW_LINE_PLACEHOLDER, "\n");
            ...
            using (var csvReader = new CSVFileReader(csvPath) { FieldDelimiter = characterSeparator.Value, WrapFields = wrapFields })
            {
                ...
                for (var r = 0; r < fileLines.Count; ++r)
                    for (var c = 0; c < fileLines[r].Count; ++c)
                        importedTranslations[r, c] = ReplaceSymbolWithNewLine(fileLines[r][c]);
